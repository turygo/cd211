...
- **Copy task**: the CloudDrive2 task that copies cloud content to local NAS storage. Its identity is the exact `(source, workspace)` pair.
- **Logical save path**: the category root requested by qBittorrent and persisted as `save_path`. It remains the qBittorrent-facing path, not the physical destination for a new isolated copy.
- **Workspace path**: the exact per-download physical staging directory `<logical save path>/.cd211/<lowercase 40-character hash>`, persisted as `workspace_path`. An empty value is the explicit legacy shared-layout boundary.
- **Content path**: the exact verified local file or torrent root exposed as qBittorrent `content_path`; for isolated rows it is inside `workspace_path`.
...
2. New rows persist a workspace exactly at `<save_path>/.cd211/<hash>`; the path is clean, absolute, strictly below the logical save path, and tied to the same lowercase 40-character hash. Legacy rows persist an empty workspace path.
3. `content_path` is generated by CD211. For isolated rows it is inside `workspace_path`; legacy rows with an empty `workspace_path` retain the historical save-path boundary.
4. The logical save path and workspace path are frozen at submission. Symlink aliases are resolved only for containment checks; a later retarget outside the configured root fails closed rather than rewriting durable paths.
5. Before use, CD211 enforces exactly `03770` (sticky and setgid) on the logical save path, exactly `02750` (setgid, owner-writable, group-traversable) on `.cd211`, and exactly `02770` (setgid and group-writable) on each hash workspace, without changing ownership or group.
6. `.cd211` is a globally reserved exact path component for every configured local root and every logical download or category save root. A clean absolute logical path, or its fully resolved canonical path at a filesystem boundary, is invalid if any component is named exactly `.cd211`; near-names such as `.cd211-backup` and ordinary symlink aliases without the reserved component remain valid, and internally generated `workspace_path` values are exempt. Distinct hashes stay isolated when different logical roots resolve to the same physical save root. This deliberately rejects logical roots and symlink targets that may previously have been valid. The SQL migration preflight and later write guards enforce literal logical paths. Before any configured runtime is activated at process startup or in a settings hot swap, a read-only filesystem preflight resolves symlinks and validates the configured local root, every category save root, and each download save root selected by `state != 'DELETED' OR (content_path != '' AND delete_files_requested = 0)`; generated `workspace_path` and `content_path` values are not external roots and are not checked. A failure prevents the new runtime generation from becoming active.
7. A workspace cannot equal or contain another row's logical save path, and a logical save path cannot equal or be nested below another row's workspace. Shared logical save-path parents remain allowed. Workspace deletion validates the tree and directory identity, removes children through the workspace's own `os.Root`, revalidates identity, then removes only the empty hash directory. It never renames a workspace or creates a quarantine directory; existing `.quarantine/<hash>` remnants from older versions are cleaned through the same rooted deletion path. An interrupted deletion leaves remaining contents in place for the next cleanup attempt. Directory handles constrain traversal but do not lock out concurrent writers, same-UID processes, privileged processes, or mount changes; concurrent writes may cause cleanup to fail instead of completing.
8. A canonical torrent info hash identifies at most one non-deleted download record.
9. API handlers persist intent before returning success. They do not depend on an in-memory queue.
10. Every external CloudDrive2 operation is safe to retry after an unknown result.
11. No database transaction remains open during a network or filesystem call.
12. Sonarr and Radarr can continue polling completed items until they explicitly remove them.
13. A category change after submission changes the label only; it does not silently relocate existing data.
14. Permanent workflow failure is never reported as completion.
15. Logs and the Web UI never expose tracker passkeys or raw authenticated URLs.
16. A domain event and its matching delivery fan-out are committed in the same transaction as the download mutation that produced them; fan-out never blocks or delays the mutation.
17. Webhook delivery is at-least-once. Consumers must deduplicate by event ID, and replay reuses the original event ID and payload.
18. Webhook receiver URLs, signing secrets, and request bodies are never written to logs.
...
Local cleanup failures retain their existing durable problem code and safe API message. Warning logs record the short hash, workflow operation, allowlisted filesystem operation, and numeric errno with its system description when available; paths, filenames, and raw wrapped error messages are not logged. Missing targets are handled idempotently by the filesystem layer, not by discarding every `ENOENT` at the scheduler boundary.
...
The logical save path is the shared category root requested by qBittorrent. For each new download CD211 creates one isolated workspace at the exact `<logical save path>/.cd211/<lowercase hash>` and sends that workspace—not the logical root—to CloudDrive2 as the copy destination. The verified `content_path` is the exact file or torrent root inside that workspace. An empty `workspace_path` marks a row retained in the legacy shared layout; those rows continue using their historical save-path behavior until cleaned up. All four services must see the shared root and every nested workspace at the same absolute path. A later category or root setting change never relocates a frozen row.
...
- `GET /api/v1/downloads/{hash}` returns the query model: persisted uppercase state, projected progress, `row_version` as version, terminal/outcome mapping, sanitized error, timestamps, additive nullable `workspace_path`, content path, and `links`. `workspace_path` is null for legacy shared-layout rows and the exact per-hash workspace for isolated rows. Additive nullable `error_code` and `next_retry_at` fields carry the durable problem code and the scheduled retry time when present; existing response semantics remain compatible. A `DELETED` row remains queryable; a never-existing hash is 404. `submission_uri`, raw sources, tracker passkeys, and cloud credentials are never exposed.
...
- Downloaded content is on persistent staging mounts. The configured local root is the logical save-path root; each isolated download owns a nested `.cd211/<hash>` workspace beneath it.
- CloudDrive2, CD211, Sonarr, and Radarr must mount the same host staging directory at the same absolute path (normally `/downloads`). CloudDrive2 must expose that exact path in its own virtual filesystem because CD211 submits each isolated workspace path as the copy destination. Verify with `FindFileByPath` before deploying; no extra volume is needed for `.cd211/<hash>` because it is nested below the existing mount.
...
- CD211 creates or prepares the logical save path with exactly `03770` (sticky+setgid), the `.cd211` namespace with exactly `02750` (setgid, owner-writable, group-traversable but not group-writable), and each hash workspace with exactly `02770` (setgid and group-writable). It preserves the configured owner and group. CloudDrive2, Sonarr, and Radarr must all run in the `PGID` group, because CloudDrive2 writes copied content and the other two read it from the same mount.
- `.cd211` is globally reserved as an exact component of configured local roots and logical download/category save roots; near-names such as `.cd211-backup` remain valid. Both the clean absolute path and the fully resolved canonical target must satisfy the rule, except that CD211's generated workspace and content paths necessarily enter the reserved namespace. Migration `00015_download_workspace.sql` checks literal stored path components and aborts atomically before schema changes or backfill if any category save path, or any non-deleted or retained download save path, violates the rule; it cannot detect a legacy symlink alias whose canonical target violates it. Before process startup or a settings hot swap activates a configured runtime, a filesystem-aware preflight checks the configured local root, all category save roots, and download save roots selected by `state != 'DELETED' OR (content_path != '' AND delete_files_requested = 0)`. A legacy alias that resolves through `.cd211` or outside the configured local root fails activation; a failed hot swap leaves the current runtime active. Rename or repoint the offending alias, or reconfigure and correct the affected local root or stored category/download path, then restart CD211 or apply Settings again. Deleted downloads that no longer retain local content are exempt from these preflight scans but remain subject to later write guards. Deleting local files quarantines an owner-controlled workspace entry and revalidates its inode before recursive removal, preserving the parent and other hash workspaces. CloudDrive2 should use umask `0002` or `0007` so copied content remains group-writable.
...
| `paths.local_root` | Shared staging root visible at the same absolute path to CloudDrive2, CD211, Sonarr, and Radarr; category save paths are descendants built from this root and a UI-managed relative subpath, or the root itself for the default category |
...
The reconciler submits a copy from the persisted cloud source path to the row's frozen workspace path. New isolated rows use the exact `<save_path>/.cd211/<hash>` destination; legacy rows with an empty `workspace_path` retain the save path as their copy destination.

Crash recovery first looks for an existing copy task with the exact `(source path, workspace path)` identity. An existing task is adopted instead of duplicated, and a crash after submission never triggers destructive preflight.

The destination name is reserved within an isolated workspace once the finished offline task supplies the safe name; different hashes may use the same name because their workspaces are distinct. Legacy rows retain the historical cross-row destination reservation. Pre-copy collision detection clears only the current row's workspace candidate: uploaded torrents keep strict expected verification, while a magnet treats any safe existing regular file or directory at the destination as a collision regardless of type.
...
CD211 resolves the candidate beneath the row's physical root: `workspace_path` for isolated rows, or the frozen `save_path` for legacy rows. It verifies:

- the path exists;
- it is a regular file or directory;
- its real path remains under the allowed row root;
...
For isolated downloads, qBittorrent `save_path` remains the logical category root for compatibility, while `content_path` is the verified path inside that download's workspace. The native API and Web detail additionally expose `workspace_path`; this additive field is null for legacy rows.
...
CD211 resolves the evaluated canonical absolute `savePath` without changing the filesystem, then durably registers it as a disabled category before creating a missing directory through the local-root boundary. A destination conflict therefore returns before any filesystem side effect. Successful preparation enables the requested category; a filesystem failure leaves the disabled reservation in place for a safe retry. The resolved path may equal the configured local root (the default category) or be a strict descendant of it; symbolic-link escapes are rejected.
...
Before `COMPLETED`, `content_path` is empty. At `COMPLETED`, it is the verified absolute file path for a single-file torrent or verified torrent-root directory for a multi-file torrent. In an isolated row this path is inside the per-hash workspace; the API continues to return the logical category root as `save_path`.

`save_path` is normalized as a directory path and must not equal the completed `content_path`; it remains a logical qBittorrent root, not a promise that content is directly under that root.
...
The endpoint must find a newly accepted torrent immediately because Sonarr or Radarr may query it shortly after add. Its `save_path` is always the logical category root; the per-hash workspace is an additive native/API and Web detail field, not a replacement for qBittorrent's compatibility projection.
...
`deleteFiles=true` additionally removes the current row's local content. For isolated rows this means deleting the entire hash workspace (including partial copy output) while retaining the shared `.cd211` parent and every other hash workspace. Legacy rows with null `workspace_path` keep the historical cleanup under their frozen save path.

Deleting the 115 cloud copy is a separate explicit CD211 action and is not implied by qBittorrent `deleteFiles`.

Local deletion must not follow a path outside the configured staging root or the row's isolated workspace. A failed safety check enters an operator-visible deletion error and leaves the content untouched.
...
- `setSavePath` updates a pristine stopped/accepted row's logical `save_path` and derived `workspace_path` atomically using row-version CAS and path safety. `setLocation` does the same for all requested stopped/accepted rows atomically; mixed states or destination conflicts return `409`.
...
- `delete` persists removal intent; it does not delete the 115 cloud copy. Local deletion removes only the current row's isolated workspace (or follows legacy save-path cleanup when the workspace is null).
...
Migration `00015_download_workspace.sql` adds nullable `workspace_path`. An empty value is deliberate legacy compatibility: existing completed, copying, verifying, and cleanup rows remain byte-for-byte on their historical paths. Only safe pre-copy rows with no copy/content evidence receive the exact derived isolated path; a legacy failed row cuts over only after its old copy is cancelled and its own candidate is safely removed. Before changing the schema or backfilling data, migration preflight atomically aborts if any category `save_path`, or any download `save_path` whose row is not `DELETED` or has a non-null `retained_until`, contains a literal component named exactly `.cd211`. The SQL check cannot detect a legacy symlink alias whose fully resolved target contains the reserved component. Before a configured runtime is activated at process startup or during a settings hot swap, read-only filesystem preflight validates the configured local root, every category `save_path`, and every download `save_path` selected by `state != 'DELETED' OR (content_path != '' AND delete_files_requested = 0)` against its canonical target and configured-root confinement. A violation prevents the new runtime from becoming active; a failed hot swap leaves the current generation active. The operator must rename or repoint the offending alias, or reconfigure and correct the corresponding local root or stored category/download path, then restart or apply Settings again. Deleted downloads that no longer retain local content are exempt only from these preflight scans; a later insert or update touching their logical save path is guarded. No migration moves completed content.
...
save_path                  text not null        -- frozen logical qBittorrent root
workspace_path             text                 -- exact per-hash workspace; null is legacy
destination_name           text                 -- durable live reservation inside the row workspace
...
content_path               text                 -- exact verified path; inside workspace when isolated
...
For legacy rows with an empty `workspace_path`, `(save_path, destination_name)` remains unique while the row is not `DELETED`; `.cd211` is reserved and cannot be a legacy destination. Every inserted or updated download logical `save_path` and category `save_path` must also remain a clean absolute path with no component named exactly `.cd211`; this guard applies to deleted rows as well. Isolated rows instead enforce non-empty `workspace_path` uniqueness and may reuse a `destination_name` across hashes because each name is scoped within its own workspace. Category roots may not equal or be nested below any live workspace, and a workspace may not equal or contain another row's logical save path. These symmetric containment rules prevent one workflow's cleanup from owning another workflow's staging root while allowing shared logical save-path parents. A row's workspace is released only when removal reaches `DELETED`; `deleteFiles=false` retains the workspace and content, while `deleteFiles=true` removes the workspace before that transition.
...
- **Revoke** clears the persisted plaintext, marks the row inactive, and increments `row_version`, immediately invalidating the key. Revoking an already absent or inactive key is idempotent.
...
Recovery lists copy tasks and matches the exact `(source, workspace)` identity. It adopts an existing copy task before submitting another, and skips destructive preflight after a durable submission boundary.
...
The database unique key on canonical hash is the final duplicate-submission guard. Legacy rows retain a partial unique index on `(save_path, destination_name)`; isolated rows use non-empty `workspace_path` uniqueness and scope destination names within that workspace, so distinct hashes may safely share a name. The exact `.cd211` component is reserved in every logical download/category save path, `.cd211` is also reserved as a legacy destination, and symmetric logical-save/workspace containment guards reject cross-row nesting while allowing shared logical save-path parents. CloudDrive2 matching by exact source and workspace paths is the external idempotency guard.
...
- **Delete local files** is allowed only for CD211-owned content. Isolated rows own and remove their complete hash workspace; null-workspace legacy rows retain the frozen save-path safety boundary.
...
Successful cancellation is recorded durably so a later removal does not repeat it. `deleteFiles=true` treats a cleared preflight or any submitted copy-task state as ownership evidence and idempotently removes only the current row's workspace, including partial copy output; the shared `.cd211` parent is retained.
...
- the logical save path (qBittorrent/Sonarr root), the per-hash task workspace or localized legacy marker, and the exact verified content path;
...
2. The operator username is fixed. The password is chosen during first-run setup — no default exists — and is stored in SQLite only as a salted PBKDF2-SHA256 hash. The CloudDrive2 credential and outbound webhook secrets are stored plaintext because CD211 must send them to their destination services (Sections 8.1 and 12.7). Both API credentials also retain plaintext so authenticated operators can reveal and copy them after refresh or restart; protect the database and its backups. Settings masks these credentials by default and provides Show/Hide controls. Digest-only legacy credentials remain valid until manually revoked and regenerated; their original plaintext cannot be recovered.
...
7. `content_path` points to the exact local file or torrent root and differs from `save_path`; isolated rows place it beneath `workspace_path`, while legacy rows retain their historical save-path layout.
...
11. Delete with files removes only the current row's safe content/workspace and never another hash's workspace.
