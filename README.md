...
`qbt_` 密钥只授权 `/api/v2`，可在 Web 设置页生成和撤销。完整密钥默认隐藏，登录后可随时点击“显示”查看或复制，刷新页面或重启服务后仍可获取。数据库保存密钥明文、摘要、提示和元数据，请保护数据库及其备份。旧版仅保存摘要的密钥继续有效，但需手动撤销并重新生成一次，才能查看完整值。qBittorrent 会话与 Web 会话相互隔离。
...
令牌完整明文默认隐藏，登录后可在设置页随时点击“显示”查看或复制，刷新页面或重启服务后仍可获取。数据库保存明文、摘要、提示和元数据，请保护数据库及其备份。旧版仅保存摘要的令牌继续有效，但需手动撤销并重新生成一次，才能查看完整值。以 `cd211_api_` 开头的令牌只用于 `/api/v1`；独立的 `qbt_` 密钥也不能用于 `/api/v1`。原生 API 不提供重试、取消或删除端点，这些操作请在 Web 界面完成。
...
进程日志同时输出到 stderr（Docker 用户可继续使用 Dozzle），并写入数据库所在目录的 `logs/` 子目录。日志按 UTC 日期写入 `cd211-YYYY-MM-DD.jsonl`，保留最近三个月。完成配置后，操作员可在 Web 界面的“日志”页面按日期、级别和全文搜索查看，日期筛选最多支持最近三个月；页面不会提供实时 tail 或下载。日志会删除凭据、Cookie、Tracker/passkey、URL 路径和查询参数，但仍应把日志文件视为敏感运行数据。
