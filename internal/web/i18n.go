...
var stringsEN = Strings{
	...
	SavePathLabel:      "Logical save path",
	TaskWorkspace:      "Task workspace",
	LegacyWorkspace:    "Legacy shared layout (workspace not recorded)",
	CategoryLabel:      "Category",
	FrozenNote:         "The logical save path is the qBittorrent/Sonarr root. New rows use an isolated per-hash workspace below it; these paths were frozen when the download was accepted.",
	...
	APITokenSecretUnavailable: "The token value is unavailable for this legacy row. Revoke it and generate a new token.",
...
var stringsZH = Strings{
	...
	SavePathLabel:      "逻辑保存路径",
	TaskWorkspace:      "任务工作区",
	LegacyWorkspace:    "旧版共享布局（未记录工作区）",
	CategoryLabel:      "分类",
	FrozenNote:         "逻辑保存路径是 qBittorrent/Sonarr 使用的根目录。新任务会在其下使用按哈希隔离的工作区；这些路径在任务提交时固定，之后修改根目录或分类也不会移动已有数据。",
	ProgressLabel:      "进度",
	...
	APITokenSecretUnavailable: "此旧令牌没有可显示的原文。请先撤销，再生成新令牌。",
